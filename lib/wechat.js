...
class Wechat extends WechatCore {
  ...
  _getContact(Seq = 0) {
    let contacts = [];
    return this.getContact(Seq).then(res => {
  ...
  start() {
    debug('启动中...');
    return this._login().then(() => this._init()).catch(err => {
      debug(err);
      this.emit('error', err);
      this.stop();
    });
    ...
    data.forEach(msg => {
      Promise.resolve().then(() => {
        ...
        if (msg.MsgType === this.CONF.MSGTYPE_STATUSNOTIFY) {
          let userList = msg.StatusNotifyUserName.split(',').filter(UserName => !this.contacts[UserName]).map(UserName => {
            return {
              UserName: UserName
            };
          });
